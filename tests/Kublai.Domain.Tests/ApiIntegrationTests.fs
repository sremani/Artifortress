...
let private findRepoRoot () =
    let rec loop (currentPath: string) =
        if File.Exists(Path.Combine(currentPath, "Kublai.sln")) then
            Some currentPath
        else
...
type ApiFixture() =
    ...
    let oidcIssuer = "https://integration-oidc.kublai.local"
    let oidcAudience = "kublai-api"
    let oidcSharedSecret = "integration-phase7-oidc-secret"
    let oidcRoleMappings = "groups|af-admins|*|admin"
    let samlIdpMetadataUrl = "https://integration-idp.kublai.local/metadata"
    let samlExpectedIssuer = "https://integration-idp.kublai.local/issuer"
    let samlServiceProviderEntityId = "urn:kublai:integration:sp"
    ...
    let initialize () =
        ...
        | Some repoRoot ->
            let apiDll = Path.Combine(repoRoot, "src", "Kublai.Api", "bin", "Debug", "net10.0", "Kublai.Api.dll")
...
type Phase1ApiTests(fixture: ApiFixture) =
    ...
    member _.``P7-06 saml acs exchange rejects assertion with mismatched issuer`` () =
        ...
        let samlResponse =
            buildSignedSamlResponseXml
                "https://wrong-idp.kublai.local/issuer"
                fixture.SamlServiceProviderEntityId
    ...
    member _.``ER-501 audit export returns digest and preserves request correlation id`` () =
        ...
        let exportDigest = exportResponse.Headers.GetValues("X-Kublai-Export-SHA256") |> Seq.head
    ...
    member _.``ER-504 compliance evidence pack exposes legal holds approvals and audit digest`` () =
        ...
        try
            ...
            Assert.True(evidenceResponse.Headers.Contains("X-Kublai-Compliance-Pack-SHA256"))
