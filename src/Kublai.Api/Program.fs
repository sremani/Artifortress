...
let private extractScopeValuesFromPayload (payload: JsonElement) =
    ...
    [ readStringOrArrayClaim "scope"
      ...
      readStringOrArrayClaim "kublai_scopes" ]
    |> List.collect id
...
let private buildSamlServiceProviderMetadataXml
    ...
    let idpMetadataExtension =
        ...
        | Some url ->
            $"<md:Extensions><af:IdpMetadataUrl xmlns:af=\"urn:kublai:saml:1.0\">{xmlEscape url}</af:IdpMetadataUrl></md:Extensions>"
        | None -> ""
            ...
            | Ok principal ->
                ...
                | Ok () ->
                    ...
                    | Ok(governancePolicy, tenantRoleBindings, legalHolds, approvals, auditRecords) ->
                        ...
                        ctx.Response.Headers.["X-Kublai-Compliance-Pack-SHA256"] <- digest
        ...
        Func<HttpContext, IResult>(fun ctx ->
            ...
            | Ok principal ->
                ...
                | Ok entries ->
                    ...
                    ctx.Response.Headers.["X-Kublai-Export-SHA256"] <- digest
