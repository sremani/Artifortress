...
Usage:
  kublai-admin --url <api-url> [--token <pat-or-federated-token>] <command>
  KUBLAI_URL and KUBLAI_TOKEN may be used instead of --url/--token.

Global options:
  --url <url>                    API base URL. Default: KUBLAI_URL or http://localhost:5000
  --token <token>                Bearer token. Default: KUBLAI_TOKEN
...
"""
    ...
    let private collectGlobalOptions (argv: string array) =
        ...
        let defaults =
            { BaseUrl = getEnv "KUBLAI_URL" |> Option.defaultValue "http://localhost:5000"
              Token = getEnv "KUBLAI_TOKEN"
              BootstrapToken = getEnv "KUBLAI_BOOTSTRAP_TOKEN"
              CorrelationId = getEnv "KUBLAI_CORRELATION_ID"
              GovernanceApprovalId = getEnv "KUBLAI_GOVERNANCE_APPROVAL_ID"
              Args = [] }
