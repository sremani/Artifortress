...
- `deploy/helm/kublai/values-lke-preprod.example.yaml`
...
|---|---|---|---|
| `ConnectionStrings__Postgres` | `Host=localhost;Port=5432;Username=kublai;Password=kublai;Database=kublai` | yes | Primary DB connection string. |
...
| `ObjectStorage__AccessKey` | `kublai` | yes | Object-store access key. |
| `ObjectStorage__SecretKey` | `kublai-secret` | yes | Object-store secret key. |
| `ObjectStorage__Bucket` | `kublai-dev` | yes | Bucket for blob storage. |
...
| Key | Default | Required in Prod | Notes |
|---|---|---|---|
| `ConnectionStrings__Postgres` | `Host=localhost;Port=5432;Username=kublai;Password=kublai;Database=kublai` | yes | Worker DB connection string. |
...
|---|---|---|---|
| `POSTGRES_USER` | `kublai` | DB scripts | Postgres user for admin scripts. |
| `POSTGRES_DB` | `kublai` | DB scripts | Default DB target. |
...
| `DRILL_DB` | `kublai_drill` | `scripts/phase6-drill.sh` | Drill restore DB name. |
...
| `SAML_SP_ENTITY_ID` | `urn:kublai:phase7:sp` | `scripts/phase7-demo.sh` | Service provider entity id for SAML demo audience checks. |
...
- For Helm cloud deployments, prefer `secrets.create=false` with
  `secrets.existingSecretName` pointing at a Secret created by the environment
