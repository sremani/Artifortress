# artifortress.com Production Hosting Plan
...
deploying Artifortress at `artifortress.com`.
...
This matches the current Artifortress support envelope:
...
- zone: `artifortress.com`
- proxied DNS for web/API traffic
- `A` or `CNAME` from `artifortress.com` to the cloud load balancer
- `api.artifortress.com` if API and website are separated
...
`artifortress.com` production.

Use one bucket per environment:

- `artifortress-prod-blobs`
- `artifortress-staging-blobs`
- optional `artifortress-prod-backups` if database backups are copied to object
  storage
...
- access keys scoped to Artifortress buckets only
...
- HTTPS ingress for `artifortress.com`
...
- Grafana dashboard from `deploy/grafana-artifortress-operations-dashboard.json`
...
- customer support tooling outside the Artifortress stack
...
- DNS: `dev.artifortress.com` only if a shared cloud dev environment is needed
...
- DNS: `preprod.artifortress.com` and optionally
  `api-preprod.artifortress.com`
...
Prod is the customer-facing `artifortress.com` environment.
...
Do not route `artifortress.com` production traffic until:
...
- monitoring provider
- whether `artifortress.com` serves product UI, API, or both
- whether API should live at `api.artifortress.com`
